...
Cada agente está em `.Codex/agents/<nome>.md`. Invoque via Task tool conforme escopo.
...
- `whatsapp/` — envio via n8n/Evolution API, retry com backoff, cron jobs (`@nestjs/schedule`), idempotência por `event_id`.
